...
static id aspect_add(id self, SEL selector, AspectOptions options, id block, NSError **error) {
    ...
    aspect_performLocked(^{
        if (aspect_isSelectorAllowedAndTrack(self, selector, options, error)) {
            AspectsContainer *aspectContainer = aspect_getContainerForObject(self, selector); //selector方法注册一个aspectContainer obj
            identifier = [AspectIdentifier identifierWithSelector:selector object:self options:options block:block error:error]; //工厂方法，block 创建一个identifier
            if (identifier) {
                [aspectContainer addAspect:identifier withOptions:options]; //block identifier放入selector aspectContainer的前中后数组中
...
static void aspect_prepareClassAndHookSelector(NSObject *self, SEL selector, NSError **error) {
    NSCParameterAssert(selector);
    Class klass = aspect_hookClass(self, error); //forwardInvocation替换成hook的前中后
    ...
    if (!aspect_isMsgForwardIMP(targetMethodIMP)) {
        ...
        // We use forwardInvocation to hook in. //forwardInvocation和selector再替换，替换两次。。
...
static void aspect_swizzleForwardInvocation(Class klass) {
    ...
    // If there is no method, replace will act like class_addMethod. //forwardInvocation替换成__ASPECTS_ARE_BEING_CALLED__
...
static void __ASPECTS_ARE_BEING_CALLED__(__unsafe_unretained NSObject *self, SEL selector, NSInvocation *invocation) {
    ...
    // Before hooks. -- invoke传给objectContainer里的每个aspect id 去执行。。执行block
    ...
    if (objectContainer.insteadAspects.count || classContainer.insteadAspects.count) {
        ...
        do {
            if ((respondsToAlias = [klass instancesRespondToSelector:aliasSelector])) {
                [invocation invoke]; //直接执行invocation了。。。。
                break;
            }
